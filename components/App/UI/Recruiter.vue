<template>
  <div class="recruiter-box small-box">
    <v-icon class="my-12 text-gold" size="99px"
      >mdi-emoticon-cool-outline</v-icon
    >

    <p class="text-dark mt-6 mb-12 pa-0 text-h6 text-thin">
      <span>
        Me interesa unirme a <b>Oro Bótico</b> para <b>distinguir</b> y
        <b>destacar</b> mi perfil de <b>Trader Profesional</b>.
      </span>
    </p>

    <div class="mb-9">
      <v-btn to="/auth/new" rounded outlined elevation="3" class="bg-gold">
        <span>Me anoto!</span>
        <v-icon class="ml-3">mdi-account-plus</v-icon>
      </v-btn>
    </div>

    <div class="mb-9 mx-auto x-small-box">
      <br />
      <small class="text-dark"
        ><b>¿ESTAS LISTO PARA CAMBIAR TU REALIDAD FINANCIERA?</b></small
      >
    </div>
  </div>
</template>
